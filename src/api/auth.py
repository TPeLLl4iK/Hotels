...
from src.exceptions import ObjectAlreadyExistsException, UserAlreadyExistsHTTPException
...
async def register_user(
    db: DBDep,
    data: UsersRequestAdd = Body(
        openapi_examples={
            "1": {
                "summary": "Обычный пользователь",
                "value": {"login": "ivan_petrov", "password": "strongpassword123"},
            },
            "2": {
                "summary": "Админ",
                "value": {"login": "admin", "password": "adminpass456"},
            },
        }
    ),
):
    ...
    try:
        data = await db.users.add(new_user_data)
...
async def login_user(
    ...
    if not user:
        raise HTTPException(
            status_code=401, detail="Пользователь с таким логином не найденр"
        )
